import userAvatar from "../../../../assets/user-avatar.png";
import starIcon from "../../../../assets/star-icon.png";
import nextArrow from '../../../../assets/next.png'
import prevArrow from "../../../../assets/left-arrow.png";

export const ContentData = {
  title: "Відгуки",
  starIcon: { src: starIcon, alt: "star icon" },
	sliderArrows:{nextArr: {src: nextArrow, alt: 'next slide arrow'}, prevArr: {src: prevArrow, alt: 'prev slide arrow'}},
  reviews: [
    {
      id: "1",
      avatar: { src: userAvatar, alt: "user avatar" },
      name: "Darina Verba",
      tourInfo: {
        name: `Різдвяний Відень`,
        date: `(24 - 26 грудня)`,
        rate: "5.0",
      },
      title: "Magna etiam tempor!",
      descr:
        "Diam donec adipiscing tristique risus nec. In hendrerit gravida rutrum quisque non tellus. Viverra mauris in aliquam sem. Fermentum posuere urna nec tincidunt praesent semper feugiat. Pharetra convallis posuere morbi leo urna molestie at elementum. Augue neque gravida in fermentum et sollicitudin ac orci phasellus. Eget gravida cum sociis natoque penatibus et magnis dis. Eu ultrices vitae auctor eu augue. ",
    },
    {
      id: "2",
      avatar: { src: userAvatar, alt: "user avatar" },
      name: "Tsvetomir Milanovic",
      tourInfo: {
        name: `Різдвяний Відень`,
        date: `(24 - 26 грудня)`,
        rate: "5.0",
      },
      title: "Magna etiam tempor!",
      descr:
        "Diam donec adipiscing tristique risus nec. In hendrerit gravida rutrum quisque non tellus. Viverra mauris in aliquam sem. Fermentum posuere urna nec tincidunt praesent semper feugiat. Pharetra convallis posuere morbi leo urna molestie at elementum. Augue neque gravida in fermentum et sollicitudin ac orci phasellus. Eget gravida cum sociis natoque penatibus et magnis dis. Eu ultrices vitae auctor eu augue. ",
    },
    {
      id: "3",
      avatar: { src: userAvatar, alt: "user avatar" },
      name: "Darina Verba",
      tourInfo: {
        name: `Різдвяний Відень`,
        date: `(24 - 26 грудня)`,
        rate: "4.0",
      },
      title: "Magna etiam tempor!",
      descr:
        "Diam donec adipiscing tristique risus nec. In hendrerit gravida rutrum quisque non tellus. Viverra mauris in aliquam sem. Fermentum posuere urna nec tincidunt praesent semper feugiat. Pharetra convallis posuere morbi leo urna molestie at elementum. Augue neque gravida in fermentum et sollicitudin ac orci phasellus. Eget gravida cum sociis natoque penatibus et magnis dis. Eu ultrices vitae auctor eu augue. ",
    },
    {
      id: "4",
      avatar: { src: userAvatar, alt: "user avatar" },
      name: "Tsvetomir Milanovic",
      tourInfo: {
        name: `Різдвяний Відень`,
        date: `(24 - 26 грудня)`,
        rate: "5.0",
      },
      title: "Magna etiam tempor!",
      descr:
        "Diam donec adipiscing tristique risus nec. In hendrerit gravida rutrum quisque non tellus. Viverra mauris in aliquam sem. Fermentum posuere urna nec tincidunt praesent semper feugiat. Pharetra convallis posuere morbi leo urna molestie at elementum. Augue neque gravida in fermentum et sollicitudin ac orci phasellus. Eget gravida cum sociis natoque penatibus et magnis dis. Eu ultrices vitae auctor eu augue. ",
    },
  ],
};
